 ...
 * either a Google Docs link (/document/d/ID) or a Drive file link
 * (/file/d/ID) -- either way we just need the id out of it.
 */
...
export async function getGuidelines(): Promise<string | undefined> {
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch guidelines file (${res.status}). Make sure it's shared as ` +
  ...
  // confirmation interstitial -- either way, that HTML isn't guidelines
  // content, so surface a clear error instead of silently using it as one.
  if (contentType.includes("text/html")) {
    throw new Error(
      "Got an HTML page instead of the guidelines file content -- check that it's " +
        'shared as "Anyone with the link can view", and that GUIDELINES_DOC_ID points ' +
        "at the file itself, not a folder."
    );
  }
